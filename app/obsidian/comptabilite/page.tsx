    ...
    <div className="page-container">
      ...
      <div style={{display:"flex",gap:"0.5rem",marginBottom:"1.25rem"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:"0.3rem"}}>
          {[0,1,2,3,4].map(i=><button key={i} onClick={()=>setWeekOffset(i)} style={{padding:"0.3rem 0.625rem",borderRadius:"var(--radius)",cursor:"pointer",fontFamily:"'Inter',sans-serif",fontSize:"0.72rem",fontWeight:weekOffset===i?700:400,background:weekOffset===i?"var(--gold-muted)":"var(--surface)",border:`1px solid ${weekOffset===i?"rgba(var(--gold-rgb), 0.4)":"var(--border)"}`,color:weekOffset===i?"var(--gold)":"var(--text-muted)"}}>{i===0?"Cette sem.":`-${i}sem`}</button>)}
        </div>
      </div>
      {tab==="apercu"&&<>
      ...
      {tab==="saisie"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.5rem"}}>
        <div className="card"><div className="section-title" style={{marginBottom:"1rem"}}>Nouvelle opération</div>
          ...
          <div className="form-group"><label>Type d'argent</label><div style={{display:"flex",gap:"0.35rem"}}>{["sale","propre","mixte"].map(t=><button key={t} onClick={()=>setForm(f=>({...f,type_argent:t}))} style={{padding:"0.3rem 0.625rem",borderRadius:"var(--radius)",cursor:"pointer",fontFamily:"'Inter',sans-serif",fontSize:"0.75rem",fontWeight:form.type_argent===t?700:400,background:form.type_argent===t?"var(--gold-muted)":"var(--surface)",border:`1px solid ${form.type_argent===t?"rgba(var(--gold-rgb), 0.4)":"var(--border)"}`,color:form.type_argent===t?"var(--gold)":"var(--text-muted)"}}>{t}</button>)}</div></div>
