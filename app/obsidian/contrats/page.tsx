...
export default function ContratsPage(){
  ...
  async function save(){if(!supabase||!form.titre)return;setSaving(true);const membres=membresInput.split(",").map(s=>s.trim()).filter(Boolean);const payload={...form,membres_affectes:membres,created_by:user?.nom||""};if(editId){const{data,error}=await supabase.from("obsidian_contrats").update(payload).eq("id",editId).select().single();if(error){alert("❌ Erreur: "+error.message);setSaving(false);return;}setContrats(c=>c.map(x=>x.id===editId?data:x));showToast("Mis à jour");notifyDiscordUpdate("contrats",data.discord_message_id,buildContratEmbed(data));}else{const{data,error}=await supabase.from("obsidian_contrats").insert([payload]).select().single();if(error){alert("❌ Erreur: "+error.message);setSaving(false);return;}setContrats(c=>[data,...c]);showToast("Contrat créé");notifyDiscordCreate("contrats",buildContratEmbed(data)).then(msgId=>{if(msgId&&supabase){supabase.from("obsidian_contrats").update({discord_message_id:msgId}).eq("id",data.id).then();setContrats(c=>c.map(x=>x.id===data.id?{...x,discord_message_id:msgId}:x));}});}setShowForm(false);setEditId(null);setForm({...EMPTY});setMembresInput("");setSaving(false);}
  async function saveRapport(){if(!supabase||!showRapport)return;await supabase.from("obsidian_contrats").update({rapport}).eq("id",showRapport.id);const updated={...showRapport,rapport};setContrats(c=>c.map(x=>x.id===showRapport.id?updated:x));setShowRapport(null);showToast("Rapport sauvegardé");notifyDiscordUpdate("contrats",updated.discord_message_id,buildContratEmbed(updated));}
  async function changeStatut(id:string,statut:string){if(!supabase)return;await supabase.from("obsidian_contrats").update({statut}).eq("id",id);const c0=contrats.find(x=>x.id===id);const updated={...c0,statut};setContrats(c=>c.map(x=>x.id===id?updated:x));if(c0)notifyDiscordUpdate("contrats",c0.discord_message_id,buildContratEmbed(updated));}
  async function del(id:string){if(!supabase)return;const c0=contrats.find(x=>x.id===id);await supabase.from("obsidian_contrats").delete().eq("id",id);setContrats(c=>c.filter(x=>x.id!==id));showToast("Supprimé");if(c0)notifyDiscordDelete("contrats",c0.discord_message_id,buildContratEmbed(c0));}
