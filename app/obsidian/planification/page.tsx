...
export default function PlanificationPage(){
  ...
  async function save(){if(!supabase||!form.titre||!form.date)return;setSaving(true);const membres=membresInput.split(",").map(s=>s.trim()).filter(Boolean);const payload={...form,membres,created_by:user?.nom||""};if(editId){const{data,error}=await supabase.from("obsidian_planification").update(payload).eq("id",editId).select().single();if(error){alert("❌ Erreur: "+error.message);setSaving(false);return;}setEvents(e=>e.map(x=>x.id===editId?data:x));showT("Mis à jour");}else{const{data,error}=await supabase.from("obsidian_planification").insert([payload]).select().single();if(error){alert("❌ Erreur: "+error.message);setSaving(false);return;}setEvents(e=>[...e,data].sort((a,b)=>a.date.localeCompare(b.date)));showT("Événement créé");}setShowForm(false);setEditId(null);setForm({...EMPTY});setMembresInput("");setSaving(false);}
